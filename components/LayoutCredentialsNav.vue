  ...
  <aside class="system-nav">
    ...
    <div class="operation-nav-list" :style="{height:`${mainHeight()}px`}">
      <ul class="system-flows text-center">
        <li v-for="credential in availableCredentials" :key="credential.id" :class="isActiveViewClass(credential.id)">
          <nuxt-link :to="`/credentials/edit?id=${credential.id}`" class="flow-item w-80 pointer">
            <div class="operation-title relative">
              <span class="text-ellipsis">{{ credential.properties.name || 'Unnamed credential' }}</span>
            </div>
          </nuxt-link>
        </li>
      </ul>
      <ul class="system-flows text-center">
        <li>
          <nuxt-link class="add-action" to="/credentials/add">
...
<script>
export default {
  name: 'LayoutCredentialsNav',
  data () {
    return {}
  },
  computed: {
    availableCredentials () {
      return this.$store.state.availableCredentials
    }
  },
  methods: {
    mainHeight () {
